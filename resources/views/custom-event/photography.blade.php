...
<div class="min-h-screen flex items-center justify-center p-4 relative">
  ...
  <div class="glass-card p-10 pt-8 rounded-3xl w-[600px] min-h-[500px] flex flex-col shadow-xl relative z-10">
      ...
      <!-- Go Back -->
      <a href="{{ route('custom-event.catering') }}" 
...
<div class="w-full mb-8">
    ...
    <div class="w-full bg-white/30 rounded-full h-2 overflow-hidden">
      ...
      <!-- Form -->
      <form method="POST" action="{{ route('custom-event.finalize') }}" class="space-y-6 flex flex-col flex-grow">
          @csrf
          ...
          <!-- Submit Button -->
          <button type="submit" formaction="{{ route('custom-event.xtraoptions') }}" 
                  class="w-full py-3 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-semibold hover:scale-105 transition transform shadow-lg mt-2">
              Next: Extra Options →
          </button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateEstimatedCost() {
        ...
        document.getElementById('estimated-cost').innerText = `Estimated Cost: ৳${cost}`;
        // Total estimated cost can sum up from localStorage or other steps
        let total = cost;
