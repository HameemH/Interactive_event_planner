...
<div class="min-h-screen flex items-center justify-center p-4 relative">
 ...
 <div class="fixed inset-0 w-full h-full">
  ...
  <div class="glass-card p-10 pt-8 rounded-3xl w-[650px] min-h-[600px] flex flex-col shadow-xl relative z-10">
      ...
      <!-- Back Button -->
      <a href="{{ route('custom-event.stage') }}" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTotalEstimatedCost() {
        let total = 0;
        ['event_venue','event_seating','event_stage','event_catering','event_photography','event_extra'].forEach(key => {
            const data = JSON.parse(localStorage.getItem(key) || '{}');
            if (data.total_catering_cost) total += data.total_catering_cost;
