...
@section('content')
    ...
    <div class="min-h-screen flex items-center justify-center p-6 relative">
      ...
      <a href="{{ route('customize.event') }}" class="absolute top-4 left-4 text-gray-700 hover:text-indigo-600 font-semibold">
        &#8592; Go Back
      </a>
      <div class="flex-1 flex flex-col justify-between">
        <!-- Form data is handled by JavaScript and localStorage, no need for actual form submission -->
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        dateInput.addEventListener('change', function() {
          ...
          <span id="total-estimated-cost" class="px-6 py-3 bg-indigo-800 text-white rounded-full text-lg">Total Estimated Price: $0</span>
                ...
                <!-- "Next" Button -->
                <a href="{{ route('customize.seating') }}" class="block w-full py-3 rounded-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white font-semibold hover:scale-105 transition transform mt-6 shadow-lg text-center">
                    Next: Select Seating
                </a>
            </div>
        </div>
